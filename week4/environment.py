...
class Environment:
        ...
        for i in range(len(world_map)):
            for j in range(len(world_map[i])):
                if world_map[i][j] == 's':
                    ...
                    world_map[i][j] = utils.Robot((j, i), len(world_map), len(world_map[0]))
                elif world_map[i][j] == '*':
    ...
    def move_to(self, position, to):
        current_x, current_y = position
        to_x, to_y = to

        robot_temp = self.world[current_y][current_x]
        self.world[current_y][current_x] = ' '
        self.world[to_y][to_x] = robot_temp
        return True

    def extinguish(self, cords):
        cords_x, cords_y = cords
        self.world[cords_y][cords_x] = ' '
